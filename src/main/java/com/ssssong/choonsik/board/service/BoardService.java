...
public class BoardService {
    ...
    @Transactional
    public boolean registPost(BoardDTO boardDTO) {

        log.info("[ProductService] registPost Start ===================================");
        log.info("[ProductService] boardDTO : " + boardDTO);
        ...
        try {
            replaceFileName = FileUploadUtils.saveFile(IMAGE_DIR, imageName, boardDTO.getBoardImage());
            log.info("[ProductService] replaceFileName : " + replaceFileName);

            boardDTO.setBoardImageUrl(replaceFileName);

            log.info("[ProductService] insert Image Name : "+ replaceFileName);

            result = boardMapper.registPost(boardDTO);
...
//        return boardMapper.registPost(boardDTO) > 0;
        return result > 0;
    }
    ...
    public String updatePost(BoardDTO boardDTO) {
        ...
        try {
            String oriImage = boardMapper.selectPostByCode(String.valueOf(boardDTO.getBoardCode())).getImgUrl();
            ...
            if(boardDTO.getBoardImage() != null){
                ...
                // 이미지 변경 없을 시
                boardDTO.setBoardImageUrl(oriImage);
            }

            result = boardMapper.updatePost(boardDTO);
    ...
    @Transactional
    public boolean deletePost(Long boardCode) {

        return boardMapper.deletePost(boardCode) > 0;
    }
